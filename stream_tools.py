...
def get_livestream_url(url):
    stream = os.popen(f'\"{YTDL_PATH}\" -g -f best "{url}"') # '-f best', 95, -g, -e
    live_url = stream.read().strip()
    if "video" in live_url: # Weak verification of valid link
        return live_url
    else:
        print("Could not retrieve livestream URL")
        return ""
...
def get_stream_screenshot(url):
    ...
    stream = os.popen(f'\"{FFMPEG_PATH}\" -hide_banner -loglevel error -i "{url}" -f image2 -frames:v 1 "{stream_img_outname}"')
...
def get_screen_from_yt_link(url):
    ...
    if stream_url != "":
        return get_stream_screenshot(stream_url)
    else:
        return ""
